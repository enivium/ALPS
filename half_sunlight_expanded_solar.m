% half_sunlight_expanded_solar.m
% ALPS simulation script for surface missions with the first half in sunlight
% with larger solar arrays
...
num_runs = 100;
num_years = 5;
output_dir = 'output/half_sunlight_expanded_solar/';
time_step = 0.25;
long_time_step = 24;
coast_length = 24;
...
for run = 1:num_runs
    ...
    for yr = 1:num_years
        ...
        sunlight_length = surface_mission_length / 2;
        darkness_length = surface_mission_length - sunlight_length;

        for step = 1:(sunlight_length/time_step)
        ...
        for step = 1:(darkness_length/time_step)
            ss_load = load.generate_steady_state_load(mean_surface_load);
            transient_load = load.generate_transient_load();
